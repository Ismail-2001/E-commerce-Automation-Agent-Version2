...
import { Package, Search, AlertTriangle, TrendingUp, RefreshCw, Zap, X, Copy, Check } from 'lucide-react';
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-lg h-[600px] flex flex-col sticky top-6">
            {!selectedProduct ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-gray-400">
                ...
                <p className="font-medium">Select a product to launch<br />AI Analysis</p>
              </div>
            ) : (
              <>
                ...
                <div className="flex-1 p-6 overflow-y-auto">
                  {loading ? (
                    <div className="h-full flex flex-col items-center justify-center space-y-3">
                      ...
                      <span className="text-sm font-medium text-gray-500 animate-pulse">Running inventory diagnostics...</span>
                    </div>
                  ) : aiInsight ? (
                    <div className="space-y-6">
                      <div className={`p-4 rounded-xl border-l-4 ${aiInsight.type === 'restock' ? 'bg-amber-50 border-amber-500' :
                          aiInsight.type === 'dead_stock' ? 'bg-rose-50 border-rose-500' :
                            'bg-green-50 border-green-500'}`}>
                        <div className="flex items-start gap-3">
                          {aiInsight.type === 'restock' && <AlertTriangle className="w-5 h-5 text-amber-600 mt-0.5" />}
                          {aiInsight.type === 'dead_stock' && <TrendingUp className="w-5 h-5 text-rose-600 mt-0.5" />}
                          {aiInsight.type === 'ok' && <Check className="w-5 h-5 text-green-600 mt-0.5" />}
                          <div>
                            <h3 className={`text-sm font-bold uppercase tracking-wide mb-1 ${aiInsight.type === 'restock' ? 'text-amber-800' :
                                aiInsight.type === 'dead_stock' ? 'text-rose-800' : 'text-green-800'}`}>
