  ...
  signUp: async (email, password, merchantName) => {
    ...
    try {
      ...
      if (!url) {
        ...
        const mockUser = { id: 'mock-user-123', email, app_metadata: {}, user_metadata: {}, aud: '', created_at: '' } as User;
        const mockMerchant = { id: 'mock-merchant-123', name: merchantName, slug: merchantName.toLowerCase() };
        set({ user: mockUser, merchant: mockMerchant, loading: false });
        // Make sure we load the mock data since we aren't connected to supabase
  ...
  signIn: async (email, password) => {
    ...
    try {
      ...
      if (!url) {
        ...
        const mockUser = { id: 'mock-user-123', email, app_metadata: {}, user_metadata: {}, aud: '', created_at: '' } as User;
        const mockMerchant = { id: 'mock-merchant-123', name: 'Demo Store', slug: 'demo-store' };
        set({ session: {} as Session, user: mockUser, merchant: mockMerchant, loading: false });
        useDataStore.getState().loadMockData();
        return;
      }
